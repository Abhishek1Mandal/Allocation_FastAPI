...
def process_dataframe(file_path, selected_column, desired_columns=None):
    ...
    # Handle 'acceptanceStatus' column: Set all rows to "pending"
    if "acceptanceStatus" not in df.columns:
        df["acceptanceStatus"] = "pending"
    else:
        df["acceptanceStatus"] = "pending"  # Override all values to "pending"

    # Handle 'assignedStatus' column: Set all rows to "Assigned1"
    if "assignedStatus" not in df.columns:
        df["assignedStatus"] = "Assigned1"
    else:
        df["assignedStatus"] = "Assigned1"  # Override all values to "Assigned1"
